...
from Modelos_TeoriaColas import ClassPICS

class PICSApp:
    def __init__(self, root, port=5000):
        self.root = root
        self.port = port
        self.root.title(f"Modelo PICS (M/M/1) - Puerto {self.port}")
    ...
    def create_interface(self):
        """Crea la interfaz específica para el modelo PICS"""
        self.clear_window()
    ...
    def calculate_model(self):
        """Calcula los resultados para el modelo PICS"""
        try:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--port', type=int, default=5000, help='Puerto para la comunicación entre aplicaciones')
    args = parser.parse_args()
    
    root = tk.Tk()
    app = PICSApp(root, port=args.port)
    root.mainloop()